...
public class SampleController {
	...
	public ResponseEntity<String> ex07() {
		// ResponseEntity<String> 응답해더(200 ok, 404, 304)를 만들어 리턴한다.
		log.info("SampleController.ex07 메서드 실행....");
		
		String msg = "{\"name\":\"kkw\"}"; //json으로 {name : kkw}
				HttpHeaders  header = new HttpHeaders();
				header.add("Content-Type", "application/jsonl;json;chaset=UTF-8");
				// 헤더에 타입추가용
				
				return new ResponseEntity<String>(msg, header, HttpStatus.OK);
				// msg에는 json데이터/ header에는 json 백업/상태값은 ok -> 200
	}
